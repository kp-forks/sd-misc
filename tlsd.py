...
def run_aria2c_download():
 ...
 if(params['ckpt_url']):
  ...
  if(params['ckpt_url'].find('huggingface')):    
...
full_precision_str=f"{params['user_arguments']} --share --disable-safe-unpickle --xformers --enable-insecure-extension-access --opt-sub-quad-attention --opt-channelslast --api --multiple --listen --gradio-queue"
